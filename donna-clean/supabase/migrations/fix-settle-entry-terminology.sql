-- Migration: Update settle_entry RPC function to use new terminology
-- Purpose: Fix settlement to create Cash IN/Cash OUT instead of Cash Inflow/Cash Outflow
-- Date: 2025-11-29

-- Drop the old function
DROP FUNCTION IF EXISTS settle_entry(UUID, UUID, NUMERIC, DATE);

-- Create the updated settle_entry function with new terminology
...
BEGIN
  ...
  IF v_entry.entry_type = 'Credit' THEN
    ...
    -- INSERT cash entry with NEW TERMINOLOGY
    INSERT INTO entries (
      user_id,
      entry_type,
      category,
      ...
      p_user_id,
      -- ✅ FIXED: Use 'Cash IN' and 'Cash OUT' instead of 'Cash Inflow' and 'Cash Outflow'
      CASE WHEN v_is_inflow THEN 'Cash IN' ELSE 'Cash OUT' END,
      v_entry.category,
...
-- Add comment
COMMENT ON FUNCTION settle_entry IS 'Atomically settles a Credit or Advance entry. For Credit entries, creates a Cash IN/OUT entry and updates the original entry within a single transaction. Uses new terminology: Cash IN/Cash OUT.';
