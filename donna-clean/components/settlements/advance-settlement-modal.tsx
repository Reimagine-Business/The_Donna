...
import { type AdvanceGroup } from './pending-advances-dashboard'
...
interface AdvanceSettlementModalProps {
  advance: AdvanceGroup | null
  onClose: () => void
  onSuccess?: () => void
}
...
export function AdvanceSettlementModal({
  ...
  useEffect(() => {
    if (advance && advance.items.length === 1) {
      setSelectedItemId(advance.items[0].id)
    }
  }, [advance])

  // Auto-set full amount when item selected
  useEffect(() => {
    if (selectedItemId && advance) {
      const item = advance.items.find((i) => i.id === selectedItemId)
      if (item) {
        const remaining = item.remaining_amount ?? item.amount
        setAmount(remaining)
      }
    }
  }, [selectedItemId, advance])

  if (!advance) return null

  const selectedItem = advance.items.find((i) => i.id === selectedItemId)
      ...
      <div className="w-full max-w-2xl bg-background rounded-lg shadow-2xl max-h-[90vh] overflow-y-auto p-4 md:p-6">
        ...
        <div className="mb-4">
          <div className="flex items-center gap-2">
            <h2 className="text-xl font-bold">Settle Advance for {advance.name}</h2>
        ...
        <div className="space-y-3">
          {/* Advance Summary - Compact */}
          <div className="flex items-center justify-between py-3 px-4 bg-muted/50 rounded-lg">
            <div>
              <span className="font-semibold">{advance.name}</span>
              <span className="text-sm text-muted-foreground ml-2">
                ({advance.itemCount} {advance.itemCount === 1 ? 'item' : 'items'})
              </span>
            </div>
          ...
          {!selectedItem && advance.items.length > 1 && (
