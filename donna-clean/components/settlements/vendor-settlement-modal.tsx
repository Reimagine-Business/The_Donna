...
import { type VendorGroup } from './pending-bills-dashboard'
...
export function VendorSettlementModal({
  vendor,
  onClose,
  onSuccess,
  ...
  useEffect(() => {
    if (vendor && vendor.items.length === 1) {
      setSelectedItemId(vendor.items[0].id)
    }
  }, [vendor])

  // Auto-set full amount when item selected
  useEffect(() => {
    if (selectedItemId && vendor) {
      const item = vendor.items.find((i) => i.id === selectedItemId)
      if (item) {
        const remaining = item.remaining_amount ?? item.amount
        setAmount(remaining)
      }
    }
  }, [selectedItemId, vendor])

  if (!vendor) return null

  const selectedItem = vendor.items.find((i) => i.id === selectedItemId)
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 px-4">
      <div className="w-full max-w-2xl bg-background rounded-lg shadow-2xl max-h-[90vh] overflow-y-auto p-4 md:p-6">
        {/* Header */}
        <div className="mb-4">
          <h2 className="text-xl font-bold">Settle Bill for {vendor.name}</h2>
        </div>

        <div className="space-y-3">
          {/* Vendor Summary - Compact */}
          ...
          {!selectedItem && vendor.items.length > 1 && (
